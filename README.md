...
権限は **ロールのポリシー文書 1 本** で表します。コンソールの「ロール」タブは
左にロール一覧、右にその詳細という構成で、「＋ ロールを追加」で作り、ルールと
テストをフォームで編集します。保存すると文書全体が検証され、新しい版になります。
文書そのものを直接書きたい場合 (他所からの取り込みなど) は「文書を直接編集」から
切り替えられます。
