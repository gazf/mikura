...
/**
 * multipart/mixed PATCH: 複数 range を 1 リクエストで送る経路。
...
Deno.test("PATCH multipart/mixed: 複数 range を 1 リクエストで反映", async () => {
  await withTestKv(async (kv) => {
    const tokens = await setupUsers(kv);
    const fx = await ensureFixtureDir();
...
Deno.test("PATCH multipart/mixed: 大きめ range (1MB×4) を streaming で処理", async () => {
  await withTestKv(async (kv) => {
    const tokens = await setupUsers(kv);
    const fx = await ensureFixtureDir();
...
Deno.test("PATCH multipart/mixed: boundary 不在は 400", async () => {
  await withTestKv(async (kv) => {
    const tokens = await setupUsers(kv);
    const fx = await ensureFixtureDir();
