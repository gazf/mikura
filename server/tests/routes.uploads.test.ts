...
/**
 * multipart/byteranges PATCH: 複数 range を 1 リクエストで送る経路。
 * ranges は [(offset, bytes), ...] の配列を順に part として組み立てる。
 */
async function patchMultipartChunks(
  token: string,
  deviceId: string,
  uploadId: string,
  ...
  for (const r of ranges) {
    const end = r.offset + r.data.length - 1;
    parts.push(
      enc.encode(
        `\r\n--${boundary}\r\nContent-Type: application/octet-stream\r\nContent-Range: bytes ${r.offset}-${end}/*\r\n\r\n`,
      ),
    );
    parts.push(r.data);
  }
...
Deno.test("PATCH multipart/byteranges: 複数 range を 1 リクエストで反映", async () => {
  await withTestKv(async (kv) => {
    const tokens = await setupUsers(kv);
    const fx = await ensureFixtureDir();
    const target = `/${FIXTURE_DIR}/multi.bin`;
    try {
      await acquireLock(target, 1, "dev-alice");
      const start = await startUpload(tokens.alice, "dev-alice", target);
      assertEquals(start.status, 201);
      ...
      // 非連続 3 range を 1 PATCH。順序は意図的に offset 昇順でない。
      const res = await patchMultipartChunks(tokens.alice, "dev-alice", id, [
        { offset: 100, data: new Uint8Array([0xAA, 0xBB]) },
        { offset: 0, data: new Uint8Array([0x01, 0x02, 0x03]) },
        { offset: 50, data: new Uint8Array([0xCC]) },
      ]);
      assertEquals(res.status, 200);
      assertEquals(res.body?.rangeCount, 3);
      ...
      assertEquals(got.length, 102);
      assertEquals(Array.from(got.subarray(0, 3)), [0x01, 0x02, 0x03]);
      assertEquals(got[50], 0xCC);
      assertEquals(Array.from(got.subarray(100, 102)), [0xAA, 0xBB]);
      assertEquals(got[20], 0); // gap は POSIX 0 埋め
    } finally {
      await releaseLock(target, 1, "dev-alice");
      await fx.cleanup();
    }
...
Deno.test("PATCH multipart/byteranges: 大きめ range (1MB×4) を streaming で処理", async () => {
  await withTestKv(async (kv) => {
    const tokens = await setupUsers(kv);
    const fx = await ensureFixtureDir();
    const target = `/${FIXTURE_DIR}/multibig.bin`;
    try {
      await acquireLock(target, 1, "dev-alice");
      const start = await startUpload(tokens.alice, "dev-alice", target);
      assertEquals(start.status, 201);
      ...
      // 4 つの 1MB range を 8MB 跨ぐ位置に分散配置 (gap あり)。
      const ranges = [0, 2, 4, 6].map((i) => {
        const data = new Uint8Array(RANGE);
        data.fill(0x80 | i); // 値域を分けて区別
        return { offset: i * RANGE, data };
      });
      const res = await patchMultipartChunks(
        tokens.alice,
        "dev-alice",
        id,
        ranges,
      ...
      assertEquals(res.status, 200);
      assertEquals(res.body?.rangeCount, 4);

      const fin = await finalize(tokens.alice, "dev-alice", id, 7 * RANGE);
      assertEquals(fin.status, 200);

      const got = await downloadContent(tokens.alice, "dev-alice", target);
      assertEquals(got.length, 7 * RANGE);
      for (const r of ranges) {
        // 各 range の境界を 1 byte ずつサンプリングして同値性を確認。
        assertEquals(got[r.offset], r.data[0]);
        assertEquals(got[r.offset + RANGE - 1], r.data[0]);
      }
      // gap 領域 (1*RANGE..2*RANGE) は 0 埋め。
      assertEquals(got[RANGE + 100], 0);
    } finally {
      await releaseLock(target, 1, "dev-alice");
      await fx.cleanup();
    }
...
Deno.test("PATCH multipart/byteranges: boundary 不在は 400", async () => {
  await withTestKv(async (kv) => {
    const tokens = await setupUsers(kv);
    const fx = await ensureFixtureDir();
    const target = `/${FIXTURE_DIR}/bad.bin`;
    try {
      await acquireLock(target, 1, "dev-alice");
      const start = await startUpload(tokens.alice, "dev-alice", target);
      const id = start.uploadId!;
      const res = await app.fetch(
        new Request(`http://localhost/uploads/${id}`, {
          method: "PATCH",
          headers: {
            ...authHeaders(tokens.alice, "dev-alice"),
