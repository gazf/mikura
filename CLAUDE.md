...
- Authorization: **named roles stored one per KV record** (ADR-035 for the model, ADR-036 for storage; supersedes ADR-008). `server/src/policy/` holds parse/evaluate/validate/render as pure functions; `policy.service.ts` holds storage, assignment (user × role), assertions, and the in-memory compiled-policy cache. The compiled policy is built by rendering records to text, parsing, and compiling — deliberately, to keep exactly one parser and one evaluator rather than a second records-to-compiled path that would drift. `effective(user, path) = max over the user's roles of roleLevel(role, path)`, where each role resolves to its single nearest ancestor-or-self rule — order-independent and monotonic. Default deny means **invisible**; ancestors of a granted path become name-only *visible* (derived, never written). REST and WSS both filter through `checkPermission`, which is now pure computation over the cached policy — there is no `PermissionContext` and no per-check KV read. Groups are gone.
- Role edits go through `PUT /admin/roles/:name` (one role), with `POST .../enabled` and `POST .../generation` for the two switches that do **not** create a generation. Saving one role validates the **entire candidate set** — cross-role deny collisions, the assertion veto, and the admin invariant all need the whole picture. Save order: syntax → role unit tests → assignment-layer assertion veto → "at least one user holds admin `/`". A disabled role grants nothing but keeps its definition, assignments, tests (which still run), and pinned paths. `GET/PUT /admin/policy` are export/import of the text form; import replaces all roles and runs the same guards. A path named literally in any rule is **pinned**: rename and delete return 409. Startup parses the stored policy once and **refuses to start** if it fails; a policy that parses but leaves nobody holding `admin /` starts with a loud warning instead (a fresh install legitimately passes through that state). Recovery writes KV directly with the server down: `deno task seed --policy <file>` replaces the document, `deno task seed --grant-admin [--user <name>]` restores administration by installing the bootstrap policy if none exists and assigning the `admin /` role. Both exist because the HTTP recovery path needs the very admin authority that is missing.
