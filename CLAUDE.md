...
- Authorization: a **versioned policy document of named roles** in Deno KV (ADR-035, supersedes ADR-008). `server/src/policy/` holds parse/evaluate/validate as pure functions; `policy.service.ts` holds storage, assignment (user × role), assertions, and the in-memory compiled-policy cache. `effective(user, path) = max over the user's roles of roleLevel(role, path)`, where each role resolves to its single nearest ancestor-or-self rule — order-independent and monotonic. Default deny means **invisible**; ancestors of a granted path become name-only *visible* (derived, never written). REST and WSS both filter through `checkPermission`, which is now pure computation over the cached policy — there is no `PermissionContext` and no per-check KV read. Groups are gone.
- Policy edits go through `PUT /admin/policy` as **one text document**, never per-rule endpoints — "who can access what" being readable and diffable as a single artifact is the point. The console edits **per role**, master/detail: a list on the left, a form on the right that composes the selected role's block and splices it into the document over the block's `line`..`endLine` range (returned by `GET /admin/policy`), then PUTs the whole thing. Splicing rather than regenerating preserves hand-written comments and role ordering. A raw-text mode is kept for import and whole-document review. Save order: syntax → role unit tests → assignment-layer assertion veto → "at least one user holds admin `/`". A path named literally in any rule is **pinned**: rename and delete return 409. Startup parses the stored policy once and **refuses to start** if it fails; a policy that parses but leaves nobody holding `admin /` starts with a loud warning instead (a fresh install legitimately passes through that state). Recovery writes KV directly with the server down: `deno task seed --policy <file>` replaces the document, `deno task seed --grant-admin [--user <name>]` restores administration by installing the bootstrap policy if none exists and assigning the `admin /` role. Both exist because the HTTP recovery path needs the very admin authority that is missing.
